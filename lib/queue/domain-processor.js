 ...
 * 
 * Features:
 * - Auto-retry on transient errors (timeout, network issues)
    ...
    try {
            ...
            for (const savedPage of savedPages) {
                ...
                try {
                    ...
                    const result = await submitFormWithRetry(
                        submitForm,
                        savedPage.url,
                        formData, // Arg 2: formFields 
                        { // Arg 3: submissionData
                        ...
                        { // Arg 4: options
                            ...
                            campaignId: domain.campaignId,
                            freshFingerprint: true, // Default to true based on logic
                        }
                    )
        ...
        await prisma.domain.update({
            ...
            data: {
                ...
                errorMessage: failureReason, // Set error message if failed
            }
        })
