...
export async function processNextDomain() {
    ...
    try {
        ...
        console.log(`  📍 Starting with ${initialUrls.length} URLs from sitemaps`)
        ...
        console.log(`  🎯 Submission targets: Forms=${targetForms}, Comments=${(config.targetCommentsCount && config.targetCommentsCount > 0) ? config.targetCommentsCount : commentsCount}`)
        console.log(`  ✅ submitForms=${domain.campaign.submitForms}, submitComments=${domain.campaign.submitComments}`)

        if (domain.campaign.submitForms && formsCount > 0) {
            console.log(`  📝 Step 3: Submitting up to ${targetForms} forms...`)
            let submitted = 0
