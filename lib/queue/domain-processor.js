/**
 * Per-Domain Sequential Processor (Dual Database Support)
 */

import { prisma, getPrismaSupabase } from '../db.js'
...
/**
 * Process the next available domain from available databases
 */
export async function processNextDomain(retryCount = 0) {
    ...
    // Try Supabase (PostgreSQL) second if available
    const supabaseClient = getPrismaSupabase()
    if (supabaseClient) {
        const cloudResult = await processNextDomainInDb(supabaseClient, retryCount, 'SUPABASE')
...
export async function resetStuckDomains() {
    ...
    const supabaseClient = getPrismaSupabase()
    if (supabaseClient) {
        await resetStuckDomainsInDb(supabaseClient, 'SUPABASE')
    }
}
