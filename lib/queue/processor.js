...
async function processDomainAnalysis(task) {
    ...
    // Save homepage contacts (ONLY if enabled)
    if ((homepageEmails.length > 0 || homepagePhones.length > 0) && config.extractEmails !== false) {
...
async function processCrawlPages(task) {
    ...
    for (const page of pages) {
            ...
            if (page.hasForm) {
                if (domain.campaign.submitForms) {
                    if (formsQueued < targetForms) {
                        ...
                        formsQueued++
                        console.log(`  📝 Queued form submission for ${page.url}`)
                    } else {
                        console.log(`  ⚠️ Skipping form: Limit reached (${formsQueued}/${targetForms})`)
                    }
                } else {
                    console.log(`  ℹ️ Skipping form: Submission disabled in settings`)
                }
            }

            if (page.hasComments) {
                if (domain.campaign.submitComments) {
                    if (commentsQueued < targetComments) {
                        await prisma.processingQueue.create({
                            data: {
                                campaignId: domain.campaignId,
                                pageId: savedPage.id,
                        ...
                        commentsQueued++
                        console.log(`  💬 Queued comment submission for ${page.url}`)
                    } else {
                        console.log(`  ⚠️ Skipping comment: Limit reached`)
                    }
                } else {
                    console.log(`  ℹ️ Skipping comment: Submission disabled`)
                }
            }
        ...
        // Save contacts if found (ONLY if enabled)
        if ((page.contacts.email || page.contacts.phone) && campaignConfig.extractEmails !== false) {
            await prisma.extractedContact.upsert({
                where: {
                    campaignId_domainId: {
                        campaignId: domain.campaignId,
...
async function processFormSubmission(task) {
    ...
    const result = await submitForm(page.url, page.formFields, submissionData, {
        ...
        screenshots: config.saveScreenshots, // Respect screenshot setting
