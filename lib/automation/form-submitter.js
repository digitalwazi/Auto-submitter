...
async function fillFormFieldsWithBehavior(page, formFields, data, behavior = {}) {
    ...
    const commonSelectors = [
        ...
        { selectors: ['textarea[name*="message"]', 'textarea[name="body"]', 'textarea:not([name*="search"]):not([name*="comment"])'], value: data.message },
...
function getSubmitSelectors(pluginType) {
    ...
    const baseSelectors = [
        ...
        'button.submit',
        'button.send',
        'form button:not([type="reset"]):not([type="button"])', // Generic button but avoid reset/cancel if possible
    ]
